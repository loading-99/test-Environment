...
<div class="aside-box">
  <el-col >
    <el-menu
      ...
      class="el-menu-vertical-demo  listmenu">
      ...
      <div v-if="JSON.stringify($store.state.userinfo) !=='{}'">
        ...
        <el-menu-item v-for="item in $store.state.userinfo.userlist" :index="(item.id).toString()" :key="item.id" @click='touserlist(item.id)'>
        <i class="iconfont">&#xe60f;</i>
      ...
      <div v-if="JSON.stringify($store.state.userinfo) !=='{}'">
        ...
        <el-menu-item v-for="item in $store.state.userinfo.starlist" :index="(item.id).toString()" :key="item.id" @click='touserlist(item.id)'>
        <i class="iconfont">&#xe60f;</i>
        <span slot="title">{{item.name}}</span>
      </el-menu-item>
      </div>
...
.listmenu{
    ...
    .is-active{
        color:#C62F2F;
        background-color: rgba($color: #b4abab, $alpha: .2);
        span{
          color:#C62F2F;
        }
    ...
    li:hover{
        background-color: rgba($color: #dacfcf, $alpha: .2);
